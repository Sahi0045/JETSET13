...
describe('the days of a trip', () => {
  it('gives the day the last flight leaves, apart from the day it lands', () => {
    expect(lastFlightDepartureDate({ itineraries: overnight })).toBe('2026-11-15');
    expect(lastFlightDepartureDate({ itineraries: roundTripHomeOvernight })).toBe('2026-11-15');
    // tripDates keeps the landing day, which the passport rule reads.
    expect(tripDates({ itineraries: overnight })).toEqual({ firstDate: '2026-11-15', lastDate: '2026-11-16' });
    expect(tripDates({ itineraries: roundTripHomeOvernight })).toEqual({ firstDate: '2026-10-04', lastDate: '2026-11-16' });
  });

  it('falls back to the day it lands when the last flight has no departure time, and to nothing without flights', () => {
    const noDeparture = [{ segments: [{ departure: { iataCode: 'JFK' }, arrival: { iataCode: 'LHR', at: '2026-11-16T10:00:00' } }] }];
    expect(lastFlightDepartureDate({ itineraries: noDeparture })).toBe('2026-11-16');
    expect(lastFlightDepartureDate(null)).toBeNull();
  });
});
