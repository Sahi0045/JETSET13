...
describe('toClientBooking', () => {
  ...
  it('exposes the ticketing verdict fields the e-ticket helper reads', () => {
    ...
    expect(out.needs_review).toEqual({ reason: 'ticket_numbers_not_retrieved', no_confirmed_seat: false, ticket_numbers_missing: true, commit_unknown: false, unrecorded_cancellation: false });
  ...
  it('keeps only the review reason and the ticketed flag, not what the chain recorded', () => {
    ...
    expect(listed.needs_review).toEqual({ reason: 'charge not reversed after the booking failed', no_confirmed_seat: false, ticket_numbers_missing: false, commit_unknown: false, unrecorded_cancellation: false });
