...
// Flight Price Analysis - Historical price metrics
router.get('/price-analysis', async (req, res) => {
  try {
    ...
    console.log(`📊 Flight Price Analysis: ${origin} → ${destination} on ${departureDate}`);
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to get flight price analysis'
    });
  }
});
...
router.get('/inspiration', async (req, res) => {
  try {
    ...
    const result = await AmadeusService.getFlightInspirationSearch(origin, {
      departureDate,
      oneWay: oneWay === 'true',
      duration,
      nonStop: nonStop === 'true',
