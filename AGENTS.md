...
- ~~**Amadeus DNS outage (external).**~~ **Resolved by migration, Sep 2026.** The dead Self-Service REST host is gone from the codebase entirely. Flights now run on **Amadeus Enterprise Web Services (SOAP)** via `backend/services/amadeusSoap/`, against WSAP `1ASIWJETJEC` on the **PDT test** node, served from the Frankfurt host (`api.jetsetterss.com`). Search is live; **booking is off in production** (`AMADEUS_WS_BOOKING_ENABLED=false`) until Amadeus certification, and `AMADEUS_WS_AUTO_TICKET=false` because PDT has no ticketing stock. Mock flights remain deliberately disabled and every mock branch has been deleted.
  - **Amadeus certification — issue list (from the Sep 2026 session + reference-flow slides).** Our SOAP implementation matches the reference flows on session management (stateless = no `awsse:Session`; stateful = Start→InSeries→End, InSeries drops the password), the cancel flow, and Master Pricer. Outstanding items:
    - **On us (code):** (a) ticket-number **retry loop** on `PNR_Retrieve` after issuance — the number takes a moment to land in the PNR (config `AMADEUS_WS_TICKET_RETRIEVE_*`, `bookingChain.js`); (b) set `AMADEUS_WS_MIN_PAYMENT_RATIO > 0` before `AUTO_TICKET` goes true (payment-coverage guard, PR #55).
    - **On Amadeus (their side):** (a) ~~`91A` INACTIVE QUEUE BANK~~ **resolved 15 Sep 2026** — Amadeus activated the PDT queue bank; `Queue_PlacePNR` now works on queue 50 (the code default) and on 90 C0, a test queue that exists on PDT only, which we use for testing and certification — all three parts are env: `AMADEUS_WS_QUEUE_NUMBER` (default 50), `AMADEUS_WS_QUEUE_OFFICE` (default the office id), `AMADEUS_WS_QUEUE_CATEGORY` (default 0); production needs a queue from the PRD bank list; (b) **ticketing carriers** — the office can issue tickets only for carriers on its authorised list (Lufthansa yes, Air India no → `2161 PROHIBITED TICKETING CARRIER`); PDT carries the same list as PRD and adding a carrier needs an agreement with that airline; (c) **prod password/WSAP** issued only after certification (no test↔prod sync); (d) optional paid entitlements — Fare Families / Upsell, seat maps — we soft-fail today.
