...
- ~~**Amadeus DNS outage (external).**~~ **Resolved by migration, Sep 2026.** The dead Self-Service REST host is gone from the codebase entirely. Flights now run on **Amadeus Enterprise Web Services (SOAP)** via `backend/services/amadeusSoap/`, against WSAP `1ASIWJETJEC` on the **PDT test** node, served from the Frankfurt host (`api.jetsetterss.com`). Search is live; **booking is off in production** (`AMADEUS_WS_BOOKING_ENABLED=false`) until Amadeus certification, and `AMADEUS_WS_AUTO_TICKET=false` there too. PDT does issue tickets: on 15 Sep 2026, 83 of the 162 carriers on the office's list booked, ticketed and voided end to end (`docs/audits/2026-09-15-pdt-airline-sweep-round2.md`). Mock flights remain deliberately disabled and every mock branch has been deleted.
  - **Amadeus certification — issue list (from the Sep 2026 session + reference-flow slides).** Our SOAP implementation matches the reference flows on session management (stateless = no `awsse:Session`; stateful = Start→InSeries→End, InSeries drops the password), the cancel flow, and Master Pricer. Outstanding items:
    - **On us (code):** (a) ticket-number **retry loop** on `PNR_Retrieve` after issuance — the number takes a moment to land in the PNR (config `AMADEUS_WS_TICKET_RETRIEVE_*`, `bookingChain.js`); (b) set `AMADEUS_WS_MIN_PAYMENT_RATIO > 0` before `AUTO_TICKET` goes true (payment-coverage guard, PR #55); (c) per Amadeus (15 Sep 2026), when the airline's record locator is not on the PNR at commit, or issuance answers 9125, the chain signs out and retrieves and issues in NEW sessions (`issueInFreshSessions`; PDT: BI's locator was never seen in the committing session, but was seen after 12 s in a new one), with looks `AMADEUS_WS_AIRLINE_LOCATOR_POLL_MS` (default 5000) apart for up to `AMADEUS_WS_AIRLINE_LOCATOR_WAIT_MS`; (d) a segment the airline changed (status TK/TL/TN) is accepted with `PNR_AddMultiElements` optionCode 13 (change advice) before queueing, as Amadeus advised for IB4001's 1969 VERIFY ITINERARY; not proved end to end, because PDT stopped returning TK.
    - **On Amadeus (their side):** (a) ~~`91A` INACTIVE QUEUE BANK~~ **resolved 15 Sep 2026** — Amadeus activated the PDT queue bank; `Queue_PlacePNR` now works on queue 50 (the code default) and on 90 C0, a test queue that exists on PDT only, which we use for testing and certification — all three parts are env: `AMADEUS_WS_QUEUE_NUMBER` (default 50), `AMADEUS_WS_QUEUE_OFFICE` (default the office id), `AMADEUS_WS_QUEUE_CATEGORY` (default 0); production needs a queue from the PRD bank list; (b) **ticketing carriers** — the office can issue tickets only for carriers on its authorised list (Lufthansa yes, Air India no → `2161 PROHIBITED TICKETING CARRIER`); PDT carries the same list as PRD and adding a carrier needs an agreement with that airline. Fares plated on carriers issuance refused on PDT (AI GF AW BF CY EN HF JX KC KU LG NT SQ SS, and CX EK QF UL VN, which are not on the list; TK TG WY VS are not on it either but ticketed) are left out of search and refused at pricing and booking: env `AMADEUS_WS_UNTICKETABLE_CARRIERS` (unset = that list, empty = none; `backend/services/amadeusSoap/ticketingCarriers.js`); (c) **prod password/WSAP** issued only after certification (no test↔prod sync); (d) optional paid entitlements — Fare Families / Upsell, seat maps — we soft-fail today.
    - **Confirm/decide (not gaps):** fare rules now run in one session with informative pricing, as the slides draw them; FOP ordering — **changed 15 Sep 2026**: the FOP is added before pricing, on the whole PNR, because Amadeus prices OB fees from the FOP already on the PNR (Amadeus allowed after-TST only where no OB fees apply and recommended before pricing); refund-on-failure vs their "queue for manual" (deliberate business choice).
    - **Checks before the card is charged (checkout, both on by default):** the seat check sells once and signs out (`AMADEUS_WS_SEAT_CHECK_BEFORE_PAYMENT`), and in the same session prices the sold segments (`AMADEUS_WS_PRICE_CHECK_BEFORE_PAYMENT`, runs only with the seat check). A fare the airline refuses, or an adult fare higher than quoted, is a 409 `FARE_UNAVAILABLE` before payment instead of a refund after it. Without names Amadeus prices one adult, so only the adult fare is compared; child and infant fares are still checked by the booking chain. Found on PDT 15 Sep 2026: B6 37xx/39xx and some AS class-X fares were quoted by search and informative pricing but refused by PNR pricing.
- ~~Flight bookings saved with `user_id = null`~~ **Fixed:** the owner comes from the session only (`backend/utils/bookingOwner.js`), hosted checkout owns the row when it is created, and guest flight booking is off. Rows saved before that fix can still have `user_id = null` and stay out of My Trips until they are linked.
