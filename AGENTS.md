...
- ~~**Amadeus DNS outage (external).**~~ **Resolved by migration, Sep 2026.** The dead Self-Service REST host is gone from the codebase entirely. Flights now run on **Amadeus Enterprise Web Services (SOAP)** via `backend/services/amadeusSoap/`, against WSAP `1ASIWJETJEC` on the **PDT test** node, served from the Frankfurt host (`api.jetsetterss.com`). Search is live; **booking is off in production** (`AMADEUS_WS_BOOKING_ENABLED=false`) until Amadeus certification, and `AMADEUS_WS_AUTO_TICKET=false` because PDT has no ticketing stock. Mock flights remain deliberately disabled and every mock branch has been deleted.
  - **Amadeus certification — issue list (from the Sep 2026 session + reference-flow slides).** Our SOAP implementation matches the reference flows on session management (stateless = no `awsse:Session`; stateful = Start→InSeries→End, InSeries drops the password), the cancel flow, and Master Pricer. Outstanding items:
    ...
    - **On Amadeus (their side):** (a) **`91A` INACTIVE QUEUE BANK** — activate the queue bank on PDT office `SCK1S2400`; they share the office-config command and we need the **queue number + office + category** for `AMADEUS_WS_QUEUE_*` (no code change unless the category differs); (b) **ticketing stock** on PDT before `AUTO_TICKET` can go on; (c) **prod password/WSAP** issued only after certification (no test↔prod sync); (d) optional paid entitlements — Fare Families / Upsell, seat maps — we soft-fail today.
    - **Confirm/decide (not gaps):** stateless fare rules (slides draw them stateful; stateless is valid — get Amadeus's OK or wrap in a session); FOP ordering (reference adds FOP before pricing, we add it after TST — functionally equivalent); refund-on-failure vs their "queue for manual" (deliberate business choice).
