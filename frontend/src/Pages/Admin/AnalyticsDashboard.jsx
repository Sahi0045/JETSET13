...
export default function AnalyticsDashboard() {
  ...
  const statCards = [
    ...
    { label: 'Applications',  value: totals.inquiries,       color: '#6366f1', icon: '📄' },
  ...
  return (
    <div style={{ padding: '24px', fontFamily: 'Inter, system-ui, sans-serif', color: '#f1f5f9' }}>
           ...
           {['week', 'month', 'year'].map(p => (
             <button
               ...
               style={{
                 ...
                 background: period === p ? '#6366f1' : 'rgba(255,255,255,0.06)',
                 color: period === p ? '#fff' : '#94a3b8', cursor: 'pointer', fontSize: '0.85rem'
               }}
             >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 28 }}>
        {statCards.map(c => (
          <div key={c.label} style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 16, padding: '20px', border: '1px solid rgba(255,255,255,0.08)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 20 }}>
        {/* Conversion Funnel */}
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 16, padding: '24px', border: '1px solid rgba(255,255,255,0.08)' }}>
          <h3 style={{ margin: '0 0 20px', fontSize: '1.1rem' }}>Conversion Funnel</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[
              { label: 'Applications', value: funnel.applications_submitted, pct: 100, color: '#6366f1' },
              ...
              <div key={step.label}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ fontSize: '0.9rem', color: '#cbd5e1' }}>{step.label} <small style={{ color: '#64748b' }}>{step.sub}</small></span>
                  <span style={{ fontWeight: 600 }}>{step.value} <small style={{ color: step.color }}>{step.pct}%</small></span>
                </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 16, padding: '24px', border: '1px solid rgba(255,255,255,0.08)' }}>
          <h3 style={{ margin: '0 0 20px', fontSize: '1.1rem' }}>Revenue Trends ({period})</h3>
          {series.length === 0 ? <p style={{ color: '#64748b' }}>No data for this period</p> : (
            <div style={{ height: 180, display: 'flex', alignItems: 'flex-end', gap: 8, paddingBottom: 20 }}>
               {series.slice(-10).map(s => (
                 <div key={s.period} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: '0.65rem', color: '#64748b', transform: 'rotate(-45deg)', marginTop: 8 }}>{s.period.split('-').pop()}</span>
                 </div>
               ))}
            </div>
          )}
