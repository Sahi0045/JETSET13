...
// payments endpoints return JSON; normalise fetch → { ok, ...json }.
...
/**
 * Public page from the travel-agent invite email: /agent/set-password?token=…
 * Validates the token, lets the agent choose a password, then sends them to the agent login.
 */
const TravelAgentSetPassword = () => {
  ...
  useEffect(() => {
    if (!token) { setState("invalid"); setMessage("This link is missing its invitation token."); return; }
    (async () => {
      const res = await call(`payments?action=agent-invite&token=${encodeURIComponent(token)}`);
      if (res.ok && res.success) {
        setInvite({ email: res.email, name: res.name });
        setState("ready");
      } else {
        setState("invalid");
        setMessage(res.error || "This invitation link is invalid or has expired.");
      }
    })();
  }, [token]);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await call("payments?action=agent-accept-invite", {
        method: "POST",
    ...
    <div className="min-h-screen bg-[#f6f6f8] font-sans flex items-center justify-center px-4 py-12">
      <div className="bg-white rounded-3xl shadow-2xl shadow-slate-200/40 p-8 sm:p-10 max-w-md w-full">
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-black text-slate-900">Travel Agent Setup</h1>
        </div>
        ...
        {state === "invalid" && (
          <div className="text-center py-6">
            ...
            <Link to="/" className="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-100 text-slate-700 rounded-xl font-bold text-sm no-underline">
        ...
        {state === "ready" && (
          <form onSubmit={handleSubmit}>
            <p className="text-slate-500 text-sm text-center mb-6">
              ...
              <span className="font-bold text-slate-700">{invite?.email}</span> to access the agent portal.
            </p>
            {error && (
              <div className="mb-4 rounded-xl bg-red-50 text-red-700 ring-1 ring-red-200 px-4 py-2.5 text-sm font-medium">{error}</div>
            )}
            ...
            <button type="submit" disabled={submitting}
              ...
              <span className="material-symbols-outlined text-lg">{submitting ? "hourglass_empty" : "lock"}</span>
...
export default TravelAgentSetPassword;
